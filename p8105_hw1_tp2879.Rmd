...
# Discussion
The mean for the random sample is `r mean(pull(hw_1_df, sample_normal))`. 

The mean for the logical vector is `r mean(pull(hw_1_df, vec_logical))`. 
...
Applying `as.numeric()` to a character vector will give "NA" for all entries because character strings cannot be changed to numeric. 

Applying `as.numeric()` to a factor vector will give integer codes corresponding to the different factor "levels", which could compute an arithmetic mean although the mean refers to the integer codes. 